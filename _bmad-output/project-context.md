...
**Two-process desktop application:** Python GTK4 UI + TypeScript/Node sync engine, communicating over Unix socket IPC.
...
### Sync Engine (TypeScript/Node)
- **Runtime:** Node.js 22 via `org.freedesktop.Sdk.Extension.node22` (Flatpak) or system `node` (dev) — NOT Bun; CLAUDE.md Bun defaults do not apply to the engine
- **Language:** TypeScript ^5, ES2022 target, `module: "NodeNext"`, `moduleResolution: "NodeNext"`
...
- **State DB:** `better-sqlite3` — synchronous API, WAL mode mandatory; rows return plain objects but always type via interface for safety
- **Testing:** `node:test` (`describe`/`it`/`test`) + `node:assert/strict` — NOT Jest, Vitest, or `expect()`
...
- `engine/src/**/*.test.ts` → `node --test` (TypeScript)
- `engine/src/__integration__/` → `node --test` (requires manual Proton token — automated integration tests impossible due to CAPTCHA)
...
- **Local imports use `.js` extension** — TypeScript files import as `.js` (e.g., `import { foo } from "./bar.js"`); Node ESM resolves `.ts` via `tsx` in dev. Never `.ts` in import paths.
...
- **`ENGINE_PATH` dual resolution** — Flatpak: `(/usr/lib/sdk/node22/bin/node, /app/lib/protondrive/engine.js)`; dev: `(GLib.find_program_in_path('node'), <project>/engine/dist/engine.js)`; never hardcode either path
...
- **Engine integration:** `node --import tsx --test engine/src/__integration__/`
- **`tsx` loader is required** — without `--import tsx`, Node 22 cannot parse TypeScript imports and every test fails immediately
...
#### Sync Engine Tests (node:test)

- **`node:test` is NOT Jest/Vitest** — use `mock.fn()` from `node:test` (not `jest.fn()`/`vi.fn()`); check calls via `mock.calls` (not `.toHaveBeenCalledWith()`); `node:assert/strict` throws `AssertionError` on mismatch (no `.toBe()` chaining, no `expect()`)
...
GNOME SDK 50 (`org.gnome.Sdk//50`), Node.js 22, `tsx` (`npm i -g tsx`), Meson, Blueprint compiler (`blueprint-compiler`). Flatpak Builder for packaging.

#### Local Development — Two-Terminal Launch

```
# Terminal A — Engine
cd engine && npm install    # first time only
...
UI auto-spawns the engine in production (Flatpak). In dev, run them separately to see engine stdout. The UI's `ENGINE_PATH` resolution finds system `node` via `GLib.find_program_in_path()`.

#### Build & Run

- **Engine dev:** `node --import tsx engine/src/main.ts` — no compile step
- **Engine production:** `tsc` → `engine/dist/engine.js`; output is NOT self-contained — Flatpak manifest must build `better-sqlite3` native addon from source
...
- **`ci.yml` (PR gate):** `meson test` (UI) + `node --import tsx --test` (engine unit) — both must pass
...
- **CLAUDE.md Bun defaults do not apply to the engine** — engine runs Node.js 22; `bun:sqlite`, `bun test`, `Bun.serve()` are all wrong for this codebase
- **`better-sqlite3` is a native addon** — Flatpak manifest must build it from source; `tsc` output alone is not deployable
