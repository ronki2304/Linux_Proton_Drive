...
- **Bootstrap reconciliation (re-added pair)** — `computeWorkList` handles the case where both local and remote have a file but no sync state exists (pair deleted and re-added, or first sync on pre-existing files). Decision: same mtime-second + same size → `bootstrap_match` (hash local, record state, no transfer); local mtime newer → `upload` revision (pre-seeds sync state so drain routes `(defined, defined) → upload` not `(undefined, defined) → conflict`); remote mtime newer → `download`; same mtime-second + different size → `new_file_collision` (genuine content divergence). Never blindly treat all unseen files as conflicts.
- **Bootstrap upload pre-seeds sync state** — when `computeWorkList` produces an upload item with `existingNodeUid` and no prior sync state, `reconcilePair` upserts a sync state (`remote_mtime = current remote mtime, content_hash = null`) before enqueuing. This makes `processQueueEntry`'s decision table see `(state=defined, remote=defined)` → `upload` instead of `(state=undefined, remote=defined)` → `conflict`. The drain's `commitUpload` overwrites with final post-upload values.
